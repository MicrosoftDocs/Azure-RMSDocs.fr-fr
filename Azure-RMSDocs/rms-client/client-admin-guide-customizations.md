...
Si votre ordinateur déconnecté exécute la préversion du scanneur Azure Information Protection, vous devez effectuer des étapes de configuration supplémentaires. Pour plus d’informations, consultez [Restriction : Le serveur du scanneur n’est pas autorisé à se connecter à Internet](../deploy-aip-scanner-preview.md#restriction-the-scanner-server-cannot-have-internet-connectivity) dans les instructions de déploiement du scanneur.
...
Sans ce paramètre, la première sous-étiquette trouvée dans l’étiquette parente la plus élevée est appliquée à l’e-mail.
...
- **Debug** : informations complètes (paramètre par défaut).

- **Trace** : journalisation très détaillée qui a un impact sur les performances et ne doit être activée qu’à la demande du Support Microsoft. S’il vous est demandé de définir ce niveau de journalisation, pensez à fixer une valeur différente lorsque les journaux souhaités ont été collectés.
