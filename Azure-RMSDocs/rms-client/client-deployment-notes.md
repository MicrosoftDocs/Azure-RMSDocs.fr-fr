...
## <a name="ad-rms-only-limiting-the-rms-client-to-use-trusted-adrms-servers"></a>AD RMS uniquement : Limitation du client RMS à l’utilisation de serveurs AD RMS approuvés
...
Si votre compte dispose de privilèges suffisants (Administrateurs d’entreprise et administrateur local pour le serveur AD RMS), vous pouvez inscrire automatiquement un point de connexion de service (SCP) quand vous installez le serveur du cluster racine AD RMS. S'il existe déjà un SCP dans la forêt, vous devez le supprimer avant de pouvoir en inscrire un nouveau.
