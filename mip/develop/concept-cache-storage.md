...
L’application définit le type de cache lors du chargement du profil dans le `FileProfileSettings`cadre `PolicyProfileSettings`des objets `ProtectionProfileSettings` , ou. Le type de cache est statique pendant toute la durée de vie du profil. La modification d’un type de stockage de cache différent nécessite de détruire le profil existant et d’en créer un nouveau.
...
Le stockage de cache est défini via l’un des objets de paramètres de `mip::CacheStorageType` profil, via l’énumération.
...
| Ubuntu Linux      | 16,04 et versions ultérieures        | Requiert [SecretService](https://developer.gnome.org/libsecret/unstable/SecretService.html) et `LinuxEncryptedCache` l’indicateur de fonctionnalité. |
