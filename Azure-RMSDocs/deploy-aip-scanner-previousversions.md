...
1. À partir du même ordinateur Windows Server, dans votre session PowerShell, ajoutez votre premier magasin de données en exécutant la commande suivante :
    ...
    Pour obtenir d’autres exemples, utilisez la `Get-Help Add-AIPScannerRepository -examples` de commande PowerShell Help pour cette applet de commande.
...
- Mettre à jour-AIPScanner

> [!NOTE]
> La plupart de ces applets de commande sont désormais dépréciées dans la version actuelle du scanneur, et l’aide en ligne pour les applets de commande du scanneur reflète cette modification. Pour obtenir une aide sur les applets de commande antérieures à la version 1.48.204.0 du scanneur, utilisez la commande intégrée `Get-Help <cmdlet name>` dans votre session PowerShell.
